...
| `kairos_popup_clear.py` | logs\kairos_popup\latest_{before,after,manual}.png + latest.json | 노드 8788 `/popups`·`/screenshot`·`/popups/close` | **로그인 뒤 공지 팝업 원격 해제**(v11.24) — 팝업이 화면을 덮으면 hts 캡처가 통째로 `popup_blocked` 로 죽는다. ★클릭은 **열거된 팝업 사각형 안**에서만(서버가 재검사·덮임 확인), **주문·인증 계열 제목이 하나라도 뜨면 전부 중단**(ENTER 한 번이 주문 확정이 될 수 있다), 로그인 화면 무동작. 종료코드 0=없음/2=남음/3=사람확인/4=오프라인. 노드 로직은 `agent/popups.py`(+`agent/test_popups.py` 23케이스) |
