...
산출: night_cli_status.json (루트)
  run_at / rc / ok / model / log / summary(프롬프트가 찍은 NIGHT_RESULT 블록)
"""
...
STATUS = os.path.join(HERE, "night_cli_status.json")

# 프롬프트 마지막 보고 블록에서 뽑을 키(없으면 조용히 생략 — 억지로 채우지 않는다)
KEYS = ("NIGHT_RESULT", "FOR_DATE", "KOSPI", "KOSDAQ",
        "CAPTURE", "REPORT_MAIL", "PUBLISH", "NOTES")
...
def main() -> int:
    ...
    declared = (summary.get("NIGHT_RESULT") or "").lower()
    ...
    payload = {
        "_what": "전야 리서치(23:00) CLI 실행 결과. 아침 작업이 이 파일을 먼저 읽고 실패를 보고한다.",
    ...
    print("[night-status] ok=%s rc=%s declared=%s -> %s"
          % (ok, a.rc, payload["declared"], STATUS))
    if not ok:
        print("[night-status] ★전야 리서치가 정상 완료되지 않았다. 아침 작업이 이 사실을 보고할 것.")
    return 0
