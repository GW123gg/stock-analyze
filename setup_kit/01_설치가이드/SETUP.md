...
> ⚠️ **[2026-07 현황]** 이 문서는 **데몬(supervisor) 상주 모드** 세팅 가이드다. 현재 운영은
> **온디맨드 터미널-MCP 모드**(데몬 오프, 코워크가 직접 실행) — 절차는 `..\stock_research_mcp\코워크_통합지시_최종.md`,
> 파일별 코드 설명은 `CODEMAP.md` 참조. 데몬 모드로 되돌릴 때만 이 문서를 따라라.
