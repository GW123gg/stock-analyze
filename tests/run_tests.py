...
def test_new_collectors_pure():
    ...
    try:
        ...
        _NL = chr(10)
        _TBL = ("| 지수 | 방향 | 예상 |" + _NL + "|---|---|---|" + _NL
                + "| 코스피 | 하락 | -1.2% |" + _NL + "| 코스닥 | 하락 | -0.7% |")
        _good = ("# 리서치" + _NL + _TBL + _NL + "## 오늘의 픽" + _NL + "표" + _NL
                 + "오늘 파생·ETF 추천 없음 — 변동성 과열" + _NL       # v11.18: 계약의 일부
        ...
        # ★파생·ETF 는 추천이 없어도 본문에 한 줄이 있어야 한다(v11.18).
        #   실측: 분석가가 "변동성 과열이라 안 한다"는 결론까지 냈는데 부록에만 적혀
        #   독자에겐 "파생 얘기가 아예 없다"로 보였다.
        _nod = (_NL.join(["# r", _TBL, "## 오늘의 픽", "표", "## 내일 체크포인트", "- a",
                          "---", "## 부록 — 분석 근거", "선물·ETF 는 근거 부족으로 비웠다"]))
        _issd, _ = _rl.lint_report(_nod)
        check("lint: ★파생·ETF 가 부록에만 있으면 경고",
              any("파생" in i for i in _issd), str(_issd))
        _yesd = (_NL.join(["# r", _TBL, "## 오늘의 픽", "표",
                           "오늘 파생·ETF 추천 없음 — VKOSPI 77 로 프리미엄 과대",
                           "## 내일 체크포인트", "- a", "---", "## 부록 — 분석 근거", "상세"]))
        check("lint: 본문에 '추천 없음' 한 줄이 있으면 통과",
              not any("파생" in i for i in _rl.lint_report(_yesd)[0]))
        _iss2, _ = _rl.lint_report("## 오늘의 픽 F1 [5.9] force_scores T+1~2")
        # 금지토큰 4 + 구분자 없음 + 체크포인트 없음 + 파생 언급 없음(v11.18)
        #   + 표 없음(v11.30 결론 우선) = 8
        check("lint: 금지토큰 4종 + 구조 누락 4종 = 8건", len(_iss2) == 8,
